...
await Promise.all([
  ...
  mkdir(resolve(dist, "extension/icons"), { recursive: true }),
  mkdir(resolve(dist, "dashboard"), { recursive: true }),
  mkdir(resolve(dist, "collector"), { recursive: true }),
]);

const iconsDir = resolve(root, "apps/extension/icons");
const iconSources = ["icon-active", "icon-paused"];
const iconSizes = [16, 32, 48, 128];
await Promise.all(
  iconSources.flatMap((name) =>
    iconSizes.map(async (size) => {
      const svg = await readFile(resolve(iconsDir, `${name}.svg`), "utf8");
      const png = new Resvg(svg, { fitTo: { mode: "width", value: size }, background: "rgba(0,0,0,0)" }).render().asPng();
      await writeFile(resolve(iconsDir, `${name}-${size}.png`), png);
    }),
  ),
);
...
await Promise.all([
  ...
  cp(resolve(root, "apps/extension/icons"), resolve(dist, "extension/icons"), { recursive: true }),

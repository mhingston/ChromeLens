...
| Invalid or stale extension connection settings | Authenticated connection diagnostic verifies the bearer token and schema; options test current unsaved form values; popup distinguishes unavailable, authentication failure, backoff, pause, and privacy drift | An offline extension cannot verify changes until the collector is reachable |
| Privacy configuration drift | Collector-owned versioned config, restrictive local/remote union, cached offline rules, and visible stale state | A user may intentionally keep additional local exclusions beyond the canonical config |
